...
export default function RegisterMarketer() {
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    try {
      const r = await api.post('/auth/register/marketer', data);
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-12">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="card">
        ...
        <h1 className="text-2xl font-bold">הרשמה כמשווק/ת</h1>
        <p className="text-ink-500 mt-1">קבלו לינק אישי לכל אירוע והתחילו להרוויח</p>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <FormField label="שם משתמש" required error={errors.username?.message} hint="אותיות לטיניות בלבד, ללא רווחים">
            <Input dir="ltr" placeholder="ניק נייט" {...register('username', { required: 'שם משתמש חובה', pattern: { value: /^[a-z0-9_]{3,30}$/i, message: 'אותיות לטיניות, ספרות וקו תחתון בלבד (3-30)' } })} />
          </FormField>
          ...
          <button disabled={loading} className="btn-primary btn-lg sm:col-span-2 mt-2">
            {loading ? <Loader2 className="size-4 animate-spin" /> : 'התחילו להרוויח'}
          </button>
        </form>
